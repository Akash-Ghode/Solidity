...
contract Lottery {
    ...
    function random() internal view returns (uint256) {
        return
            uint256(
                keccak256(                 //This random number generation can be hacked 
                    abi.encodePacked(      //check Chainlink VRF doc  for generating random number.
                        block.difficulty,    //VRF (Verifiable Random Function)
                        block.timestamp,
                        players.length
                    )
                )
